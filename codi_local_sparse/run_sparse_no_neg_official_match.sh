...
RUN_NAME="${RUN_NAME:-sparse_no_neg_official_match_${TIMESTAMP}}"
...
echo "[run] method=sparse_no_neg gpu=$GPU_ID run_root=$RUN_ROOT" | tee "$RUN_ROOT/logs/run.log"
...
CUDA_VISIBLE_DEVICES="" python export_tensorboard_loss_plot.py \
  ...
  --title "sparse_no_neg training loss" 2>&1 | tee "$RUN_ROOT/logs/plot.log"
