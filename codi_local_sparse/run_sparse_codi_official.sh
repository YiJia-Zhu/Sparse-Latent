...
REF_LOSS_FACTOR="${REF_LOSS_FACTOR:-1.0}"
SEED="${SEED:-11}"
...
echo "[Official Sparse CODI] model=$MODEL_PATH"
echo "[Official Sparse CODI] restore=$RESTORE_PATH"
echo "[Official Sparse CODI] selector=$SELECTOR_PATH set=$SELECTOR_SET"
echo "[Official Sparse CODI] local_data_path=$LOCAL_DATA_PATH"
echo "[Official Sparse CODI] output_root=$SAVE_DIR expt=$EXPT_NAME cuda=$CUDA_VISIBLE_DEVICES"
echo "[Official Sparse CODI] report_to=$REPORT_TO"
echo "[Official Sparse CODI] epochs=$NUM_EPOCHS lr=$LEARNING_RATE exp_mode=$EXP_MODE exp_data_num=$EXP_DATA_NUM"
echo "[Official Sparse CODI] save_strategy=$SAVE_STRATEGY save_steps=$SAVE_STEPS save_total_limit=$SAVE_TOTAL_LIMIT"
echo "[Official Sparse CODI] resume_checkpoint=${RESUME_CHECKPOINT:-<auto-latest-or-empty>}"
