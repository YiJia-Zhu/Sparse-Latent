...
print(device)
print(f"[train.py] CODI_MODEL_IMPL={CODI_MODEL_IMPL}")
...
def resolve_resume_checkpoint(output_dir: str, requested_resume: str) -> Optional[str]:
    if requested_resume:
        if os.path.isdir(requested_resume):
            return requested_resume
        raise FileNotFoundError(f"Requested resume checkpoint does not exist: {requested_resume}")
...
def train():
    ...
    def load_local_training_data(local_data_path: str):
        import pandas as pd
        ...
        for row in records:
            row = dict(row)
            if "cot" in row and "answer" in row:
                normalized.append(row)
                continue
            answer_text = str(row.get("answer", ""))
            if "####" not in answer_text:
                raise ValueError(
                    "Local data rows must either contain 'cot'+'answer', or an 'answer' field with '#### final'. "
                    f"Bad row keys: {list(row.keys())}"
                )
            cot_text, final_answer = answer_text.rsplit("####", 1)
            normalized.append(
                {
    ...
    def make_supervised_data_module(tokenizer, data_args) -> Dict:
        ...
        if data_args.local_data_path:
            dataset = load_local_training_data(data_args.local_data_path)
            train_dataset = SupervisedDataset(
                data_name=data_args.data_name or "icot-local",
                raw_data=dataset,
                tokenizer=tokenizer,
                bot=model.bot_id,
                eot=model.eot_id,
        ...
        if "icot" in data_args.data_name:
            ...
            prontoqa_path = data_args.local_data_path or "data/prontoqa_train.json"
    ...
    requested_resume = os.environ.get("RESUME_CHECKPOINT", "").strip()
    resume_checkpoint = resolve_resume_checkpoint(training_args.output_dir, requested_resume)
    if resume_checkpoint:
        print(f"[train.py] Resuming from checkpoint: {resume_checkpoint}")
    else:
        print("[train.py] Starting training from scratch in this run.")
