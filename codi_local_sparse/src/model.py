...
def load_causal_lm_compat(model_wrapper_class, model_name_or_path: str, **kwargs):
    try:
        return model_wrapper_class.from_pretrained(model_name_or_path, **kwargs)
    except TypeError as exc:
        if "use_flash_attention_2" not in str(exc):
            raise
        fallback_kwargs = dict(kwargs)
        fallback_kwargs.pop("use_flash_attention_2", None)
        return model_wrapper_class.from_pretrained(model_name_or_path, **fallback_kwargs)
...
class DataArguments:
    ...
    local_data_path: Optional[str] = field(
        default=None,
        metadata={"help": "Optional local json/jsonl/parquet training data path."},
    )
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    icot_train_path: str = field(default="", metadata={"help":"Optional external training data path."})
    ...
    selective_align_path: str = field(
        default="",
        metadata={"help": "Optional selector summary.json path for sparse distillation."},
    )
    selective_align_set: str = field(
        default="selected_neg",
        metadata={"help": "Selector set name, e.g. selected_neg / selected_no_neg / random."},
    )
...
class CODI(torch.nn.Module):
    def __init__(self, model_args, training_args, lora_config):
        ...
        if model_args.full_precision:
            self.codi = load_causal_lm_compat(
                model_wrapper_class,
                self.model_name,
                torch_dtype=(
        ...
        if model_args.use_decoder:
            if model_args.decoder_path:
                self.decoder = load_causal_lm_compat(
                    model_wrapper_class,
        ...
        self.selective_align_path = getattr(training_args, "selective_align_path", "")
        self.selective_align_set = getattr(training_args, "selective_align_set", "selected_neg")
        self.register_buffer(
            "selective_align_indices",
            torch.empty(0, dtype=torch.long),
            persistent=False,
        )
        self._load_selective_align_indices()

        if self.training:
            self.init()

    def _load_selective_align_indices(self) -> None:
        if not self.selective_align_path:
            print("[CODI Official Init] sparse distillation disabled")
            return

        selector_path = os.path.expanduser(self.selective_align_path)
        ...
        if isinstance(raw, list):
            ...
            if "selected_sets" in raw and isinstance(raw["selected_sets"], dict):
                selected_sets = raw["selected_sets"]
                if self.selective_align_set not in selected_sets:
                    raise KeyError(
                        f"Selector file {selector_path} does not contain set "
                        f"{self.selective_align_set!r} under 'selected_sets'."
                    )
                indices = selected_sets[self.selective_align_set]
            elif self.selective_align_set in raw:
                indices = raw[self.selective_align_set]
            elif "indices" in raw:
                indices = raw["indices"]
            else:
                raise KeyError(
                    f"Unable to find sparse coordinate set {self.selective_align_set!r} "
                    f"in selector file {selector_path}."
                )
        else:
            raise ValueError(f"Unsupported selector file format: {selector_path}")

        indices = sorted({int(idx) for idx in indices})
        if not indices:
            raise ValueError(f"No coordinates loaded from selector file: {selector_path}")
        invalid = [idx for idx in indices if idx < 0 or idx >= self.dim]
        if invalid:
            raise ValueError(
                f"Selector indices out of range for hidden size {self.dim}: "
                f"{invalid[:8]}{'...' if len(invalid) > 8 else ''}"
            )

        index_tensor = torch.tensor(indices, dtype=torch.long, device=self.selective_align_indices.device)
        self.selective_align_indices = index_tensor
        print(
            f"[CODI Official Init] sparse distillation enabled: "
            f"{len(indices)} dims from {selector_path} "
            f"(set={self.selective_align_set})"
        )

    def _maybe_select_hidden_dims(self, student_hidden: torch.Tensor, teacher_hidden: torch.Tensor):
        if self.selective_align_indices.numel() == 0:
            return student_hidden, teacher_hidden
        indices = self.selective_align_indices.to(student_hidden.device)
        return (
            torch.index_select(student_hidden, dim=-1, index=indices),
            torch.index_select(teacher_hidden, dim=-1, index=indices),
        )
    ...
    def init(self):
        ...
        if (
            ...
            state_dict = load_state_dict_compat(self.training_args.restore_from)
            ...
            for i in range(num_latent):
                ...
                if i == num_latent - 1: # the last latent embedding
                    ...
                    for j, (out, ref_out) in enumerate(zip(outputs.hidden_states, ref_outputs.hidden_states)):
                        ...
                        out_selected, ref_selected = self._maybe_select_hidden_dims(out_selected, ref_selected)
